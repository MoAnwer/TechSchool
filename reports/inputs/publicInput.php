<?php
  ...
  if (Auth::isLog()) :
    ...
    // Total input
    $totalInputQuery = $db->query("SELECT SUM(total) AS totalInput FROM inputs");
    $totalOfInput = $totalInputQuery->fetch(PDO::FETCH_ASSOC)['totalInput'];

    // Total output
    $totalOutputQuery = $db->query("SELECT SUM(total) AS totalOutput FROM outputs");
